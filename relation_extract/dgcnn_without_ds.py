# -*- coding: utf-8 -*-
"""
# @Time    : 2019/10/31 10:55
# @Author  : xiaoxiong
# @Email   : xyf_0704@sina.com
# @File    : dgcnn_without_ds.py
# @Software: PyCharm
# DESC : 不加先验特征的关系抽取
详解参考： https://spaces.ac.cn/archives/6671
"""
...
def random_generate(d, spo_list_key):
    ...
    if r > 0.5:
        ...
        # 对上述抽中的三元组中的关系，在predicates中随机抽取一个三元组
        k = np.random.randint(len(predicates[spi[1]]))
        spo = predicates[spi[1]][k]
        # 把text中的subject与object 替换为在predicates中随机抽取的三元组中subject与object
        F = lambda s: s.replace(spi[0], spo[0]).replace(spi[2], spo[2])
        text = F(d['text'])
        # 替换subject与object，构成一个新的三元组
        ...
        while True:
            ...
            for i in idxs:
                ...
                if items:
                    ...
                    k1 = choice(k1)
                    k2 = choice(k2[k2 >= k1])  # 为了让模型学习到负样本
                    ...
                    if len(T1) == self.batch_size or i == idxs[-1]:
                        T1 = seq_padding(T1)  # 字向量
                        T2 = sent2vec(T2)     # 词向量
                        S1 = seq_padding(S1)  # subject首标注
                        S2 = seq_padding(S2)  # subject尾标注
                        O1 = seq_padding(O1, np.zeros(num_classes))  # object首标注
                        O2 = seq_padding(O2, np.zeros(num_classes))  # object尾标注
                        K1, K2 = np.array(K1), np.array(K2)  # 随机采样一个object首标注
...
class Evaluate(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f1 > self.best:
            self.best = f1
            train_model.save_weights('./model/dgcnn_best_model.weights')
        ...
        if epoch + 1 == 50 or (
            ...
            self.stage = 1
            train_model.load_weights('./model/dgcnn_best_model.weights')
            EMAer.initialize()
    ...
    def evaluate(self):
        ...
        F = open('./data/dev_pred.json', 'wb')
        for d in tqdm(iter(dev_data)):
            R = set(extract_items(d['text'][:maxlen]))
            T = set(d['spo_list'])
            A += len(R & T)
            B += len(R)
            C += len(T)
...
train_D = data_generator(train_data, batch_size=32)
evaluator = Evaluate()
